...
const
    filterStudents = (search, course, previous, next) => {
        ...
        $.ajax({
            type: 'GET',
            url: '/get-list-students-bulletin',
                ...
                if(studentsC !== 0) {
                    students.forEach((item) => {
                        html +=
                            ...
                            '<a href="#" class="primary-btn small goova-bt" data-toggle="tooltip" onclick="openModalPeriods(' + item.id + ')" title="Generar Boletin">' +
    ...
    openModalPeriods = (id) => {
        ...
        $.ajax({
            ...
            let footer =
                ...
                '<a href="#" class="primary-btn small goova-bt" data-toggle="tooltip" onclick="downloadBulletin(' + student.id + ')" title="Confirmar">' +
                '<span class="ti-save"></span>' +
                '</a>' +
                '</div>' +
                '</div>';

            universalModal('Seleccione un Periodo', html, footer );
    ...
    downloadBulletin = (id) => {
        ...
        $.ajax({
            ...
            console.log(data.s);
            window.open('/get-bulletin?s=' + data.s, '_blank');
            // http://127.0.0.1:8000/get-bulletin?id=3
        }).fail(() => {
            $("body").overhang({
                type: "error",
                message: "Oops! Se detectó un problema, intenta más tarde.",
