...
const
    ...
    filterReports = (id, period) => {
        $.ajax({
            ...
            theresSomething = (isset(reports));

            html += `
                <div class="row">
                    <div class="white-box" id="student_report_container" style="width: 100%">
                        <div class="col-12">
                            <div class="input-effect">
                                <textarea class="primary-input form-control ${(theresSomething)? `has-content` : `` }" name="general_observation" id="general_observation" cols="30" rows="10">${(theresSomething)? `${ reports.observation }` : `` }</textarea>
                                <label>
                                    Observación General
                                </label>
                                <span class="focus-border"></span>
                        ...
                        <div class="col-12 right-aligned">
                            <button type="button" class="primary-btn small icon-only" id="save_b" onclick="${(theresSomething)? `editReport(${ reports.id })` : `saveReport(${ id + ',' + period })`}">
    ...
    saveReport = (student, time) => {
        let generalObservation = $(`#general_observation`).val();

        $.ajax({
            type: 'POST',
            url: '/post-save-general-report',
            data: {
                _token,
                time,
                student,
            ...
            $(`#student_report_container`).append('<div class="success-ajax"><span class="ti-check"></span></div>');
            setTimeout(() => { $('.success-ajax').remove(); }, 3000)
            $(`#save_b`).attr(`onclick`, `editReport(${data.data})`);
        }).fail(() => {
            $("body").overhang({
                type: "error",
                message: "Oops! Se detectó un problema, intenta más tarde.",
