...
const
    ...
    filters = () =>{
        console.log("ghs")
        var val=subject.val();
        var val2=periodos.val();
        $('tr[data-subject]').each(function(){
            if (val=="" && val2=="") {
                $(this).show()
            }else
            if (val.trim() == $(this).attr('data-subject').trim() && val2.trim() == "") {
                $(this).show()
            }else
            if (val2.trim() == $(this).attr('data-value').trim() && val.trim() == ""){
                $(this).show()
                $('tr[data-subject="'+val2+'"]').show()
            }else
            if (val2.trim() == $(this).attr('data-value').trim() && val.trim() == $(this).attr('data-subject').trim()) {
                $(this).show()
                $('tr[data-subject="'+val2+'"]').show()
            }else{
                $(this).hide()
                $('tr[data-subject="'+val2+'"]').hide()
            }
        })
    },
    openModalNotes = (id) => {
        ...
        $.ajax({
            ...
            universalModal('Reporte por periodo de <strong>' + data.s.name + ' ' + data.s.last_name + '</strong>', html, '', 'modal-xlg');
            subjectsFilter(id);
            ...
            $('#period_filter').html(html).niceSelect('destroy').niceSelect();

            pageN = 1; searchN = $('#search_n'); teacher = $('#teacher_filter'); subject = $('#subject_filter'); previousN = $('#previous_n'); nextN = $('#next_n'); periodos = $('#period_filter');
